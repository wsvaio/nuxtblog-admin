...
payload.$use("刷新标签列表")(async () => refreshAsync());
payload.$use("删除")(async () => {
	await delTag({ p: { id: payload.id } });
	await refreshAsync();
});
</script>
					...
					render: rowData => [
						h(
							NButton,
							{
								...
								type: 'primary',
								onClick: () => {
									payload.$action({ showAddedit: true, addeditTitle: '修改', tagId: rowData.id });
								},
							},
							{ default: () => '修改' },
						),
